...
function optionsframework_fields() {
	...
	foreach ($options as $value) {
		...
		case "heading":
            ...
            //左侧菜单直接取  type='heading'的name,所以 name只能为英文
            //此处存在翻译上的bug,我扩展heading 上存在一个  title的列存储中文title
            ...
            $menu .= '<li><a id="'.  esc_attr( $jquery_click_hook ) . '-tab" title="' . esc_attr( $value['title'] ) . '" href="' . esc_attr( '#'.  $jquery_click_hook ) . '"><img src="' . get_template_directory_uri() . '/admin/images/' . esc_html( $value['sicon'] ) . '" width="16" height="16" border="0" align="left"> &nbsp;' . esc_html( $value['title'] ) . '</a></li>';
